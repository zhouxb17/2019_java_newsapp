...
public class ScopeStack {
    ...
    // 找到最近的Foraml或Lambda Scope, 用于visitReturn 中判断正常进行return类型判断还是将return类型加到list/Stack中，供Lambda统一判断
    public Scope nearestFormalOrLambdaScope() {
        ListIterator<Scope> iter = scopeStack.listIterator(scopeStack.size());
        while (iter.hasPrevious()) {
            var scope = iter.previous();
            if (scope.isFormalScope() || scope.isLambdaScope()) {
                return scope;
            }
        }
        return null;
